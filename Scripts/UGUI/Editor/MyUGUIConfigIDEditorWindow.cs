    ...
    public class MyUGUIConfigIDEditorWindow : EditorWindow
    {
        ...
        private void _LoadAssetFile()
        {
            ...
            if (mGroups == null)
            {
                ...
                {
                    IsFoldOut = true,
                    Name = "ELoadingIndicatorID",
                    NumDefault = 3,
                    ListID = new List<string>() { "ThreeDots", "Circle", "Search" }
                });
        ...
        private void _GenerateScript()
        {
            ...
            string loadingIndicators = string.Empty;
            for (int i = 0, countI = mGroups.ListGroup[3].ListID.Count; i < countI; i++)
            {
                loadingIndicators += "\n\t\t" + mGroups.ListGroup[3].ListID[i] + ",";
            }
